/**
 * =========================================================================================
 * Project     : ESP32 High-Pressure Aeroponics Root Chamber & VPD Controller
 * Platform    : ESP32 (Dual-Core Xtensa LX6 / FreeRTOS)
 * Framework   : Arduino IDE 2.0+
 * Author      : Muhammad Fikri
 * License     : MIT
 * Description : Automated aeroponic growing system pulsing 100 PSI misting solenoids with sub-second accuracy, calculating Vapor Pressure Deficit (VPD), and dosing nutrients.
 * =========================================================================================
 */
...
void TaskTelemetryBroadcaster(void *pvParameters) {
    ...
    for (;;) {
        if (xSemaphoreTake(g_telemetryMutex, pdMS_TO_TICKS(20)) == pdTRUE) {
            ...
            doc["system"] = "esp32-aeroponics-highpressure-controller";
...
void setup() {
    ...
    Serial.println(F("================================================================="));
    Serial.println(F(" ESP32 High-Pressure Aeroponics Root Chamber & VPD Controller"));
